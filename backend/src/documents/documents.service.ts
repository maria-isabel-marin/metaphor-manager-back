...
export class DocumentsService {
  ...
  async findByProject(projectId: string, user: RequestUser): Promise<any[]> {
    ...
    return Promise.all(
      docs.map(async (doc) => {
        ...
        // Use new gcsPath field if available, otherwise fallback to legacy fields
        const filePath = obj.gcsPath || obj.gcsPathPdf || obj.gcsPathTxt;
  ...
  /** 4) Update metadata and optionally file */
  async update(
    id: string,
    ...
    // If a new file is provided, upload it and update paths
    if (file) {
      // Delete old file if it exists
      const existingDoc = await this.docModel.findById(id).exec();
      if (existingDoc?.gcsPath) {
        await bucket
          .file(existingDoc.gcsPath)
          .delete()
          .catch(() => {
            console.warn(
              `Could not delete old file ${existingDoc.gcsPath} from GCS.`,
            );
          });
      }

      // Upload new file
      gcsPath = `documents/${Date.now()}-${file.originalname}`;
      fileType = file.mimetype.split('/')[1]; // e.g., 'pdf', 'plain'
    ...
    if (user) {
      await this.actionLogService.logAction({
        ...
        userId: user._id,
        userEmail: user.email,
        details: { title: updated.title, changes: dto, fileUpdated: !!file },
      });
    }
  ...
  async remove(id: string, user: RequestUser): Promise<void> {
    ...
    // Use new gcsPath field if available, otherwise fallback to legacy fields
    const filePath = doc.gcsPath || doc.gcsPathPdf || doc.gcsPathTxt;
    ...
    if (filePath) {
      await bucket
        ...
        .delete()
        .catch(() => {
          console.warn(`Could not delete file ${filePath} from GCS.`);
        });
    }
    ...
    for (const row of rows) {
      ...
      const am = new this.amModel({
        ...
        epistemicMappings: String(epistemicMappings || '')
          ...
          .map((s) => s.trim())
          .filter(Boolean),
        noveltyType: noveltyType || 'conventional',
        functionType: functionType || 'structural',
        status: 'under_review',
